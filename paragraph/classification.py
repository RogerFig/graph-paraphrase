...
if __name__ == '__main__':
    ...
    model = '../trained-models/assin+msr_mod_glove50'
    # model = 'trained-model/mod_'
    # test = 'features/test/features-test-all1.txt'
    print('### Extracting features ###')
    features, _ = ExtractFeatures(model='../embeddings/glove_s50.txt', input_h='../corpora/assin+msr/test/test-h.txt',
                                  input_t='../corpora/assin+msr/test/test-t.txt').extract_features()
    # test = 'baseline/test/features-test.txt'
    labels = util.get_labels('../corpora/assin+msr/labels-test.txt')
    print('### Classifying ###')
    for a in algorithms:
