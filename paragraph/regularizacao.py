...
class Regularization:
    ...
    def __init__(self, G, labels_f):
        self.G = G
        self.labels = self.read_labels(labels_f)

    @staticmethod
    def read_labels(input_f):
        return open(input_f, 'r',).readlines()

    def regulariza(self, pre_anotados=0.3, method='gfhf'):
        """
        ...
        anotados = cods[0:int(total_samples*pre_anotados)]
        ...
        if method in ['gfhf', 'llgc']:
            for key in sentence_nodes.keys():
                ...
                if t == 'train':
                    rows_train.append(
                        [id_node, F[id_node][0], F[id_node][1], train_labels[cod]])
            ...
            for key in sentence_nodes.keys():
                ...
                if t == 'train':
                    rows_train.append(
                        [id_node, F[dict_nodes[id_node]][0], F[dict_nodes[id_node]][1], train_labels[cod]])

                if t == 'test':
                    rows_test.append(
                        [id_node, F[dict_nodes[id_node]][0], F[dict_nodes[id_node]][1]])
